...
impl Delimiter {
    pub fn new_delimiter(delimiters: impl CharToVec) -> Self {
        Self {
...
mod tests {
    ...
    #[test]
    fn test_delimiter() {
        let d1 = Delimiter::new_delimiter('\'');
        ...
        let d2 = Delimiter::new_delimiter('"');
        assert_eq!(d2.get_delimiters(), &vec!['"']);

        let d3 = Delimiter::new_delimiter(['[', ']']);
        ...
        let d4 = Delimiter::new_delimiter(['!']);
        let t4 = d4.find_all("Testing a search where 'delimiter' wasn't found".to_string());
        assert!(t4.is_empty());

        let d5 = Delimiter::new_delimiter(['[', ']']);
        let d5_2 = Delimiter::new_delimiter(['[', ']', '!']);
